    ...
    pub fn new(
        ...
        match fork().expect("couldn't fork") {
            ...
            ForkResult::Child => {
                ...
                let path = CString::new(path.clone()).expect("binary path must not contain zero");
                ...
                let shm_id = format!("__AFL_SHM_ID={}", shm_file);

                //Asan options: set asan SIG to 223 and disable leak detection
                let asan_settings =
                    "ASAN_OPTIONS=exitcode=223,abort_on_erro=true,detect_leaks=0".to_string();

                let mut env = env::vars()
                    .filter(|(key, _)| {
                        key != "__AFL_SHM_ID"
                            && key != "ASAN_OPTIONS"
                            && key != "AFL_OLD_FORKSERVER"
                    })
                    .map(|(key, value)| {
                        CString::new(format!("{}={}", key, value))
                            .expect("environment variables must not contain zero")
                    })
                    .collect::<Vec<_>>();
                env.push(CString::new(shm_id).expect("RAND_2333935040"));
                env.push(CString::new(asan_settings).expect("RAND_2089158993"));
                env.push(CString::new("AFL_OLD_FORKSERVER=1").expect("RAND_413919522"));
                ...
                println!("EXECVE {:?} {:?} {:?}", path, argv, env);
                unistd::execve(&path, &argv, &env).expect("couldn't execve afl-qemu-tarce");
                unreachable!();
            }
        }
    }
