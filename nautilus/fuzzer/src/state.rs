...
impl FuzzingState {
    ...
    pub fn minimize(
        ...
        if min_simple && min_rec {
            ...
            let materialized_input = self.fuzzer.materialize_input(&input.tree, ctx)?;
            file.write_all(&materialized_input)
                .expect("Could not write minimized materialized queue entry");
            return Ok(true);
        }
