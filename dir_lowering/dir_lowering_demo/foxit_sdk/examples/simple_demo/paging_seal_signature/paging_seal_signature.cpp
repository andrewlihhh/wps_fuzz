...
WString output_directory = output_path + L"paging_seal_signature/";
...
PagingSealSignature AddPagingSealSiganture(PDFDoc pdf_doc, const char* sub_filter, bool is_on_perforation) {
  ...
  PagingSealConfig paging_seal_config(PagingSealConfig::e_PagingSealPositionLeft, (float)page_width/2, 
    0.5f, is_on_perforation, PagingSealConfig::e_PagingSealStyleMultipleSignatures);
  new_sig.SetPagingSealConfig(paging_seal_config);
  

  // Set values for the new paging seal signature.
  new_sig.SetKeyValue(PagingSealSignature::e_KeyNameSigner, L"Foxit PDF SDK");
  String new_value;
...
void AdobePPKLitePagingSealSignature(PDFDoc pdf_doc, bool use_default) {
  ...
  bool is_on_perforation = true;
  if (use_default) is_on_perforation = false;
  PagingSealSignature new_paging_seal_signature = AddPagingSealSiganture(pdf_doc, sub_filter, is_on_perforation);
  ...
  WString signed_pdf_path = output_directory + L"signed_middle_paging_seal_signature.pdf";
  if (use_default)
    signed_pdf_path = output_directory + L"signed_left_paging_seal_signature_default_handler.pdf";
...
int main(int argc, char *argv[]) {
  ...
  try {
    for (int i = 0; i < 2; i++) {
	  ...
	  if (i > 0)
		  signed_pdf_path = output_directory + L"signed_left_paging_seal_signature_default_handler.pdf";
	  PDFDoc pdf_doc1(signed_pdf_path);
